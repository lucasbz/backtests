...
func TestTwoCandleBreakout_Operations_TooFewCandles(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(10000)}
	s.Traverse(candleWithLowHigh("2010-01-04", 1200, 1242))
	s.Traverse(candleWithLowHigh("2010-01-05", 1200, 1242))

	if ops := s.Operations(); ops != nil {
		t.Errorf("Operations() = %+v, want nil (fewer than 3 candles)", ops)
	}
}

func TestTwoCandleBreakout_Operations_DropsUnclosedPositionAtEndOfCandles(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(10000)}
	...
	s.Traverse(candleWithLowHigh("2010-01-04", 1200, 1300))
	s.Traverse(candleWithLowHigh("2010-01-05", 1100, 1250))
	s.Traverse(candleWithLowHigh("2010-01-06", 1050, 1150))

	if ops := s.Operations(); ops != nil {
		t.Fatalf("Operations() = %+v, want nil (open position at end of candles must be dropped)", ops)
	}
}

func TestTwoCandleBreakout_Operations_BuysAndSellsOnTouch(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(10000)}
	...
	// 1150) -> max 1250. This 4th candle's high reaches 1250, selling.
	s.Traverse(candleWithLowHigh("2010-01-07", 1200, 1250))
...
func TestTwoCandleBreakout_Operations_WaitsUntilPriceTouchesTrigger(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(10000)}
	// min of last two lows is 1100, but the 3rd candle's low (1150) never
	// reaches it, so no buy yet.
	s.Traverse(candleWithLowHigh("2010-01-04", 1200, 1300))
	s.Traverse(candleWithLowHigh("2010-01-05", 1100, 1250))
	s.Traverse(candleWithLowHigh("2010-01-06", 1150, 1220))

	if ops := s.Operations(); ops != nil {
		t.Fatalf("Operations() = %+v, want nil (trigger price never touched)", ops)
	}

	// next window: last two lows (1100, 1150) -> min 1100. This candle's low
	// (1090) reaches it, triggering the buy.
	s.Traverse(candleWithLowHigh("2010-01-07", 1090, 1200))
...
func TestTwoCandleBreakout_Operations_ReinvestsProceedsIntoNextCycle(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(1000)}

	// cycle 1: window(c0,c1) low-min = min(200,150) = 150; c2's low (100)
	// touches it -> buy @150, qty = 1000/150 = 6.
	s.Traverse(candleWithLowHigh("2010-01-04", 200, 300))
	s.Traverse(candleWithLowHigh("2010-01-05", 150, 250))
	s.Traverse(candleWithLowHigh("2010-01-06", 100, 200))
	// window(c1,c2) high-max = max(250,200) = 250; c3's high (400) touches
	// it -> sell @250, qty 6 -> proceeds = 250*6 = 1500.
	s.Traverse(candleWithLowHigh("2010-01-07", 300, 400))
	...
	s.Traverse(candleWithLowHigh("2010-01-08", 90, 500))
	// window(c3,c4) high-max = max(400,500) = 500; c5's high (550) touches
	// it -> sell @500, qty 15.
	s.Traverse(candleWithLowHigh("2010-01-09", 400, 550))
...
func TestTwoCandleBreakout_Operations_InsufficientBalanceSkipsBuy(t *testing.T) {
	s := &TwoCandleBreakout{Balance: newMoney(500)}
	s.Traverse(candleWithLowHigh("2010-01-04", 1200, 1300))
	s.Traverse(candleWithLowHigh("2010-01-05", 1100, 1250))
	s.Traverse(candleWithLowHigh("2010-01-06", 1050, 1150))
